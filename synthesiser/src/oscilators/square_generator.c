...
#define CFG ((struct square_generator_config*)config)

void square_generator(void* config){

	struct square_generator_config parsed_config = *((struct square_generator_config*)(config));
	...
	for (int i = 0; i < MAINBUFFER_SIZE; i++) {
	  g_sound_buffer[i] += (int32_t)(fsquare_func(CFG->phasebuffer.phase) * parsed_config.amp);
