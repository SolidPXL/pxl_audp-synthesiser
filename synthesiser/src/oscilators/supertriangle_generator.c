...
#define CFG ((struct supertriangle_generator_config*)config)

void supertriangle_generator(void* config){

	struct supertriangle_generator_config parsed_config = *((struct supertriangle_generator_config*)(config));
...
//			g_sound_buffer[i] += (int32_t)(ftriangle_func(CFG->phasebuffers[j].phase) * amp);
			g_sound_buffer[i] += (int32_t)(ftriangle_func(CFG->phasebuffers[j].phase) * (CFG->amp/CFG->voices));
